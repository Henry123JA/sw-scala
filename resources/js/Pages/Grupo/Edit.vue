      ...
      <form @submit.prevent="submit" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="form.codigo_grupo" type="text" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm" required />
...
const form = useForm({
  codigo_grupo: props.grupo.codigo_grupo,
  curso_id: props.grupo.curso_id,
  docente_id: props.grupo.docente_id,
  capacidad_maxima: props.grupo.capacidad_maxima,
  nivel: props.grupo.nivel || '',
  estado: props.grupo.estado,
});

function submit() {
  form.put(buildUrl(`/grupos/${props.grupo.id}`));
}
</script>
