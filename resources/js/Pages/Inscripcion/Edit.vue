      ...
      <form @submit.prevent="submit" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" :value="inscripcion.alumno?.usuario ? `${inscripcion.alumno.usuario.nombres} ${inscripcion.alumno.usuario.apellidos}` : ''" class="mt-1 w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-500" disabled />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700">Grupo / Curso</label>
            <input type="text" :value="`${inscripcion.grupo?.codigo_grupo} - ${inscripcion.grupo?.curso?.nombre}`" class="mt-1 w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-500" disabled />
          </div>
...
const form = useForm({
  fecha_inicio_clases: props.inscripcion.fecha_inicio_clases ? props.inscripcion.fecha_inicio_clases.split('T')[0] : '',
  observaciones: props.inscripcion.observaciones || '',
});

function submit() {
  form.put(buildUrl(`/inscripciones/${props.inscripcion.id}`));
}
</script>
